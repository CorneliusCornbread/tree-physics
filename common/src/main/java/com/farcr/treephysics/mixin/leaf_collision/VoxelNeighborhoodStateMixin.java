...
public class VoxelNeighborhoodStateMixin {
    ...
    private static boolean treephysics$isSolid(BlockGetter blockGetter, BlockPos pos, BlockState state, Operation<Boolean> original) {
        if(TreeUtil.isLeaf(state) && !TreePhysicsConfig.STATIC_LEAF_COLLISION.get() && blockGetter instanceof LevelAcceleratorAccessor accessor && Sable.HELPER.getContaining(accessor.getLevel(), pos) != null) {
            return true;
        }
